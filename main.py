...
def main():
    ...
    dss_py_generate.build_snapshot_script()
    
    out_file = os.path.join(current_dir, 'output_dss_py.dss')
    if not os.path.exists(out_file):
        return
        
    # 2. Khai bao Monitor
    settings = dss_py_generate.read_settings(os.path.join(current_dir, 'settings.txt'))
